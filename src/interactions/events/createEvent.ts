import { Interaction, CommandInteractionOptionResolver, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, ButtonBuilder, ButtonStyle, GuildMemberRoleManager, CacheType, CommandInteraction } from "discord.js";
import config from "../../config.json";
import { getEventTemplate } from "../utils/eventTemplate";

export const handleCreateEvent = async (interaction: CommandInteraction<CacheType>) => {
  ...
  const slotList = slotTimestamps.map((timestamp, index) => `Slot #${index + 1} - ${timestamp}:`).join('\n');

  // Pre-built template
  const template = getEventTemplate(eventTitle, startTimeUnix, slots, peoplePerSlot, duration, description, slotList);
  ...
  export const handleCreateEventModalSubmit = async (interaction: Interaction) => {

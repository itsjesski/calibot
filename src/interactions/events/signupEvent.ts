...
export const handleSignUpButtonInteraction = async (interaction: Interaction) => {
    ...
    const peoplePerSlot = peoplePerSlotMatch ? parseInt(peoplePerSlotMatch[1], 10) : 0;
    ...
    if (rows.length > 0) {
      await interaction.reply({ content: 'Select a slot to sign up:', components: rows, ephemeral: true });
    } else {
      await interaction.reply({ content: 'All slots are full.', ephemeral: true });
    }
  };

  export const handleSlotSelectionInteraction = async (interaction: Interaction) => {
    ...
    if (slot.includes(userMention)) {
      await interaction.followUp({ content: 'You are already signed up for this slot.', ephemeral: true });
      return;
    }
    ...
    if (signUps < peoplePerSlot) {
      ...
      await interaction.followUp({ content: `You have signed up for slot ${slotIndex + 1}.`, ephemeral: true });
    } else {
      await interaction.followUp({ content: 'This slot is full.', ephemeral: true });
    }
  };